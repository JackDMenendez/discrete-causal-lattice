...
Standard Model gauge group as automorphism
    ...
    & Six sympy scripts in \nolinkurl{src/utilities/}
      (2026-05-05) verify: existing per-site $\mathbb{C}^2$
      carries $\mathfrak{so}(3,1) \oplus \mathfrak{u}(1)$ only
      (dim 7; per-site $SU(2)$ overlaps with Lorentz rotations);
      extended $\mathbb{C}^{12}$ supports the full dim-18
      direct product, all 18 generators commuting pairwise across
      factors; bipartite Wilson plaquette
      $V_1,\,-V_2,\,-V_1,\,V_2$ is gauge-invariant.  Open: exact
      Aut equality; SM-chirality coupling; explicit $1/g^2$
      prefactors.  Details:
