...
Standard Model gauge group as automorphism
    ...
    & Six \texttt{src/utilities/automorphism\_*} and
      \texttt{tick\_rule\_*} sympy scripts (2026-05-05) establish:
      discrete spatial $\mathrm{Aut}(\Gamma, V)$ is order 48
      ($B_3 \cong O_h$ abstractly, 12 elements orthogonal); existing
      RGB symmetry contributes only $\mathbb{Z}_3 \subset SU(3)$;
      per-site $SU(2)$ on $\mathbb{C}^2$ overlaps with
      Lorentz rotations (not a separate factor); the conjecture's
      direct-product structure is achievable on the extended
      $\mathbb{C}^{12} = \mathbb{C}^2 \otimes \mathbb{C}^2 \otimes
      \mathbb{C}^3$ amplitude (verified, all 18 generators commute
      pairwise across factors); trivial tensor extension preserves
      $\mathcal{A}=1$ and parity; Wilson plaquette
      $V_1, -V_2, -V_1, V_2$ is gauge-invariant.  Three structural
      questions open: exact equality of Aut, SM-chirality coupling,
      explicit $1/g^2$ prefactors
    & \texttt{STUB} \\
\hline
