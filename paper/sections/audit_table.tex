...
\caption{\small System Audit: $\Tdiamond$ predictions mapped to
...
Photon emission as $\mathcal{A}=1$ necessity
    ...
    & \texttt{exp\_19c} (no recoils across rates 0.01--0.5);
      \texttt{exp\_20} three-arm comparison did not exhibit orbital
      lock-in --- baseline two-body instability dominates
      (\texttt{exp\_12b})
    & \texttt{PART} \\
Emission-operator joint $\mathcal{A}=1$ conservation
    ...
    & \texttt{exp\_20}: joint $\mathcal{A}=1$ preserved to
      machine precision ($8.9\!\times\!10^{-16}$) by arm B (beam
      splitter); arm A (drain) violates by $1.0$; arm C trivial
    & \texttt{PART} \\
...
Two-body long-horizon stability
    ...
    & \texttt{exp\_12b}: $r_\text{peak}$ escapes from $\sim R_1$
      to grid edge by tick $\sim 2000$ on $65^3$;
      $\mathcal{A}=1$ preserved at machine precision
    & \texttt{PART} \\
Long-horizon escape mechanism
    ...
    & Standard QM has no lattice substrate to exhibit finite-volume
      effects
    & \texttt{exp\_12c}: escape at tick $\sim 140$ on
      $\text{GRID}\!\geq\!81^3$, $\sim 2000$ on $65^3$.  The $65^3$
      ``stability'' is a finite-volume artefact (boundary reflection)
    & \texttt{PART} \\
Lock-in resonance grid-independence
    & \texttt{exp\_12} chassis on $\text{GRID} \in \{65, 81, 97,
      113\}^3$, pre-escape window
    & Standard QM has no lattice substrate to vary
    & \texttt{exp\_12d\_tight}: $r$ at $k_\text{Bohr}$ in
      $[11.54, 12.30]$ across all four grids
      ($\Delta r \approx 0.8$); three of four $k_\text{min}$ within
      $\pm 0.003$ of $k_\text{Bohr}$; lock-in grid-independent
    & \texttt{PART} \\
...
Standard Model gauge group as automorphism
    ...
    & Six sympy scripts (2026-05-05) verify: existing
      $\mathbb{C}^2$ carries dim-7 $\mathfrak{so}(3,1) \oplus
      \mathfrak{u}(1)$ only; extended $\mathbb{C}^{12}$ supports
      dim-18 direct product.  Open: SM-chirality coupling, $1/g^2$,
      exact Aut equality.  Detail:
      \nolinkurl{notes/lie_algebra_automorphism_proof_sketch.md}
    & \texttt{STUB} \\
\hline
% ── Falsifiability framework ─────────────────────────────────────────────────
Multi-channel concordance (P9)
    & Single optical axis $(1,1,-1)$ inherited by kinematic,
      gauge, gravitational, and recoil-mass sectors
    & Five-channel direction agreement $(a/\lambda)^2$;
      time-dependence variants distinguish embedding scenarios
    & Derived in §\ref{subsec:p9_concordance};
      empirical test via GRB / CMB / LIGO / atomic-clock data
    & \texttt{STUB} \\
\hline
\end{longtable}
}
