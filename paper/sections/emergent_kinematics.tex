...
\item \textbf{The Clifford algebra} $\{\gamma^\mu,\gamma^\nu\} = 2g^{\mu\nu}$
    holds because the RGB vectors form a tight frame ($\sum_\text{RGB}
    \mathbf{v}\mathbf{v}^T = 3I$) — a geometric theorem, not an
    algebraic postulate.
...
\item \textbf{Rest mass} is the Zitterbewegung oscillation rate:
    ...
    the two sublattices.
\item \textbf{Rotational invariance} is automatic: the dispersion relation
    $\omega^2(\mathbf{k}) = m^2 + \tfrac{a^2}{3}|\mathbf{k}|^2$ is
    isotropic by the frame condition, without symmetry averaging.
\end{enumerate}
