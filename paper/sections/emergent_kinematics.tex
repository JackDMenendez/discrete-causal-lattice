...
\paragraph{Isotropy: frame condition and octahedral symmetry.}
The first-order expansion of $\tilde{H}_\text{RGB}$ produces a directional
gradient $(1,1,-1)\cdot\nabla$ that superficially breaks rotational
invariance.
Two complementary arguments restore it.

\emph{Fourier-space argument.}
The CMY sublattice has $\text{CMY} = -\text{RGB}$, so
$\tilde{H}_\text{CMY}(\mathbf{k}) = \tilde{H}_\text{RGB}(-\mathbf{k})^*$.
The dispersion relation of the full two-tick operator near $\mathbf{k}
= \mathbf{0}$ is
\begin{equation}
    \omega^2(\mathbf{k}) = m^2 + \frac{a^2}{3}|\mathbf{k}|^2 + O(k^4).
    \label{eq:dispersion}
\end{equation}
The first-order term $\tfrac{i}{3}(1,1,-1)\cdot\mathbf{k}$ enters
the dispersion relation quadratically; cross terms between the RGB and
CMY contributions cancel, and the frame condition $M_{ij} = 3\delta_{ij}$
eliminates all off-diagonal contributions.
The dispersion relation depends only on $|\mathbf{k}|^2$ — it is
manifestly isotropic.

\emph{Symmetry argument.}
The $\Tdiamond$ lattice has octahedral symmetry $O_h$.
Physical observables must be $O_h$-invariant, and in the continuum limit
$O_h$ enlarges to the full rotation group $\mathrm{SO}(3)$.
The directional gradient $(1,1,-1)\cdot\nabla$ averages to zero over
$O_h$; the second-order double-hop term produces the isotropic Laplacian
$\nabla^2$ via the frame condition.
This is the same mechanism by which staggered fermions in lattice
QCD~\cite{susskind1977} recover Lorentz invariance in the continuum
limit from a discretisation that breaks it explicitly at finite $a$.
